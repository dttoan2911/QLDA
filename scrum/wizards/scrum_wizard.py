...
class CreateProductBacklog(models.TransientModel):
    ...
    def create_product_backlog(self):
        vals = self.env['product.backlog'].create({
            'inherit_project_id':self.inherit_project_id.id,
            'name_backlog':self.name_backlog
        })
    # Thuộc tính bảng Create Product Backlog
    # project_id = fields.Many2one('scrum.project',string="Dự Án")
    inherit_project_id = fields.Many2one('project.project',string="Dự án")
...
class CreateSprint(models.TransientModel):
    ...
    def create_sprint_project(self):
    ...
    @api.onchange('inherit_project_id')
    def onchange_inherit_project_id(self):
        for rec in self:
            return {'domain':{'sprint_backlog_ids':[('inherit_project_id','=',rec.inherit_project_id.id),('sprint_id','=',False)]}}
